...
int drivercode3idz()
{
    ...
    system("cls");
    int N;
    cout << "Проверка на магический квардрат.\nВыберите размерность матрицы: 6, 8, 10:" << "\n";
    cin >> N;
    int** matrix = new int* [N];
    for (int** row = matrix, **endrow = row + N; row < endrow; row++) {
        *row = new int[N];
    ...
    if (fl) cout << "\nЭта матрица является магическим квадратом";
    else cout << "\nЭта матрица не является магическим квадратом";
    return 1;
}
